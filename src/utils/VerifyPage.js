...
const VerifyPage = () => {
    ...
    const handleToVerify = async () => {
        console.log(token);
        setLoading(true);
        const res = await fetch(`${process.env.REACT_APP_APIURL}/writer/verification`, {
            method: "PATCH",
            headers: {
    ...
    return (
        <div>
            <div className="flex items-center justify-center py-12 min-w-screen">
                <div className="w-96 py-8 text-center text-gray-600 bg-[#f5f5f5] shadow-xl lg:max-w-3xl rounded-3xl lg:p-12">
                    <h3 className="text-2xl">Verify Page</h3>
                    ...
                    <p>Click To Verify</p>
                    <div className="mt-4">
                        <button title='Verify Button' onClick={handleToVerify} className="px-2 py-2 text-white bg-primary rounded">Verify</button>
                    </div>
                </div>
            </div>
        </div>
